...
text_three = f"""
Ser man på de individuelle, internationale markeders vækst i Jan.-{latest_month_string[:3]}. {latest_year}, er {len(table_two.loc[(~table_two.index.isin(['Kina*', 'Indien*'])) & (table_two['Vækst i pct.'] > 0)])} 
af vores top 10 markeder ift. {YEAR_T_MINUS_ONE}. Samtidig er det kinesiske marked er {status_table_three_kina} med {table_two['Vækst i pct.'].loc[(table_two.index == 'Kina*')].iloc[0]/100:.1%}, 
imens at det indiske marked er {status_table_three_indien} med {table_two['Vækst i pct.'].loc[(table_two.index == 'Indien*')].iloc[0]/100:.1%}.
"""

pdf_bytes = create_pdf(headline, text_one, table_one, text_two, table_three)
